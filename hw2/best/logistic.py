...
def preprocess(X, age):
    X = np.concatenate( (X, np.zeros([X.shape[0], 4])), axis = 1 )
    for i in range(X.shape[0]):
        X[i][0] = age[int(X[i][0])]
        X[i][-4] = 1 if X[i][507] > 0 else 0
        X[i][-3] = 1 if X[i][210] > 0 else 0
        X[i][-2] = 0 if X[i][211] > 0 else 1
        X[i][-1] = 1 if X[i][212] > 0 else 0
    return X
...
if __name__ == '__main__':
    ...
    # with np.load(np_file) as npz:
    #    feature = npz['feature']

    # Map the age to ratio of label 1
    eps = 0.00000000001
    age = [[0, 0] for _ in range(100)]

    for x, y in zip(x_train, y_train):
        age[int(x[0])][int(y)] += 1

    for i in range(100):
        age[i] = age[i][1] / (age[i][0] + age[i][1] + eps)

    x_train = preprocess(x_train, age)
    x_test = preprocess(x_test, age)
    
    # Delete the least significant weights
    # to_d = []
    # for c in range(1, x_train.shape[1]):
    #    if c not in feature:
    #        to_d.append(c)

    # x_train = np.delete(x_train, to_d, axis = 1)
    # x_test = np.delete(x_test, to_d, axis = 1)
    #################################################
    ...
    plt.title('Accuracy')
    plt.legend(['train', 'dev'])
    plt.savefig('./best/acc.png')
    # plt.show()
    plt.clf()

    x_axis = [i for i in range(100)]
    # Age curve
    plt.bar(x_axis, age, color = 'c', edgecolor = 'm', linewidth = 0.5)
    plt.title('Age vs. Ratio of label 1')
    plt.xlabel('age')
    plt.ylabel('ratio')
    plt.savefig('./best/age.png')
    # plt.show()
    plt.clf()
